...
if [ ${CREATE_CONTAINER} -eq 1 ]; then
    ...
    for filename in ${FOLDER}/mysql*.sql; do
        ...
        filename="${filename%.*}"
        ./scripts/test_files.sh "scripts/drop_schema.sql" ${CONTAINER_NAME} ${PASSWORD}
        ./scripts/test_files.sh "dist/mysql/${filename}" ${CONTAINER_NAME} ${PASSWORD}
    done
else
    for filename in ${FOLDER}/mysql*.sql; do
        filename=$(basename -- "$filename")
        filename="${filename%.*}"
        ./scripts/test_files.sh "scripts/drop_schema.sql" ${CONTAINER_NAME} ${PASSWORD} mysql
        ./scripts/test_files.sh "dist/mysql/${filename}" ${CONTAINER_NAME} ${PASSWORD} mysql
    done
fi
